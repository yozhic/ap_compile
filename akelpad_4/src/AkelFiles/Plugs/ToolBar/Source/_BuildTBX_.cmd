...
rc %RCFLAGS% /R /DRC_VERSIONBIT=%BIT% /Fo"ToolBarExtra.res" "Resources\ToolBarExtra.rc"
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
cl %CLFLAGS% ToolBarExtra.c ToolBarExtra.res /LD /link kernel32.lib user32.lib gdi32.lib advapi32.lib comctl32.lib ole32.lib %LINKFLAGS% /OPT:NOWIN98 /MACHINE:%MACHINE% /NODEFAULTLIB /ENTRY:DllMain
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%

::### Clean up ###::
if exist ToolBarExtra.res del ToolBarExtra.res
if exist ToolBarExtra.obj del ToolBarExtra.obj
